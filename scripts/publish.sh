...
OPTIONS
  -a, --archive-path <filepath>       Path to the buildpack or extension archive (default: ${ROOT_DIR}/build/buildpack.tgz) (optional)
...
function buildpack::publish() {
  ...
  if [[ ${#targets[@]} -gt 1 ]]; then
    ...
    util::print::info "Publishing single-arch ${buildpack_type} to ${image_ref}"
    local tmp_dir
    tmp_dir=$(mktemp -d -p "${ROOT_DIR}")
    tar -xzf "${archive_path}" -C "${tmp_dir}"
